...
| Concern | Choice | Why |
|---|---|---|
| Language | Python 3.11+ | Every LLM SDK is Python-first. Easiest for a solo builder. |
...
| Live events (browser) | `GET /v1/games/{game}/stream` (SSE) | — |
| Read a completed hand | `GET /v1/games/{game}/hands/{n}` | `get_hand_result` |
...
The Phase 1 scaffold lives in [`../pokertable/`](../pokertable/README.md): engine wrapper, REST contract, browser table, four scripted bots, an Anthropic-backed LLM agent, a headless simulator and the test suite. The OpenAPI schema it serves at `/docs` is the formal contract.
